...
export function LimitGauge({ year, quarter, limit, used, remaining, isExceeded }: LimitGaugeProps) {
  ...
  const statusLabel = isExceeded ? "Przekroczony" : percentage > 80 ? "Limit blisko" : "W normie"
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
        <div>
          <CardTitle className="text-base">Limit kwartalny</CardTitle>
      ...
      <CardContent className="space-y-4">
        ...
        {!isExceeded && percentage > 0 && (
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>
              Wykorzystano {used.toFixed(2)} PLN z {limit.toFixed(2)} PLN limitu kwartalnego
            </span>
          </div>
        )}
      </CardContent>
